<div class="conatiner">
    <form action="<?php echo base_url().'admin/product/create_product';?>" method="POST" id="myForm" name="myForm"
        class="form-container mx-auto  shadow-container" style="width:80%" enctype="multipart/form-data">
        <h3 class="mb-3 text-center">Add Seafood Product</h3>
        <div class="form-group">
            <label class="control-label">Select Category</label>
            <select name="category_name" id="catname"
                class="form-control <?php echo (form_error('category_name') != "") ? 'is-invalid' : '';?>">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <?php echo (form_error('name') != "") ? 'is-invalid' : '';?>" name="name" id="name"
                        placeholder="Enter Product name" value="<?php echo set_value('name'); ?>">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea type="text" rows ="10" class="form-control my-2
                    <?php echo (form_error('about') != "") ? 'is-invalid' : '';?>" id="about" name="about"
                        placeholder="About" value="<?php echo set_value('about'); ?>"></textarea> 
        ...
        <a href="<?php echo base_url().'admin/product/index'?>" class="btn btn-secondary">Back</a>
    </form>
</div>
<script>
    ...
    const validate = () => {
        const catnameVal = catname.value.trim();
        const dishnameVal = dishname.value.trim();
        const priceVal = price.value.trim();
        const aboutVal = about.value.trim();
        const imageVal = image.value.trim();

        if (catnameVal === "--Select Category--") {
            setErrorMsg(catname, 'select category name');
        } else {
            setSuccessMsg(catname);
        }
        if (dishnameVal === "") {
